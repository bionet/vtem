...
def launch_kernel(func, block, grid, argins, timed = None, shared = None, texrefs=[], prepared = False):
    ...
    if timed is None:
        if shared is None:
            func.prepared_call(new_grid, new_block, *arg_call)
        else:
            func.prepared_call(new_grid, new_block, *arg_call, shared_size = shared)
        
    else:
        if shared is None:
            a = func.prepared_timed_call(new_grid, new_block, *arg_call)
        else:
            a = func.prepared_timed_call(new_grid, new_block, *arg_call, shared_size = shared)
        if timed == 1:
            return a()
        else:
            time = a()
