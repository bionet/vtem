...
#and support various operation on it.
...
class PitchArray(object):
    ...
    def reshape(self, shape, inplace = False):
        ...
        func.prepared_call(self._grid, self._block, self.shape[0], _pd(self.shape), result.shape[0], _pd(result.shape), result.gpudata, result.ld, self.gpudata, self.ld)
        return result
